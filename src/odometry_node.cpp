...
void OdometryNode::LoadInitStateYAML_(const std::string &bag_filename)
{
  ...
  if (config["initial_pose"]) {
    if (config["initial_pose"][bag_filename]["position"] && config["initial_pose"][bag_filename]["position"]["x"]
        && config["initial_pose"][bag_filename]["position"]["y"]) {
    ...
    if (config["initial_pose"][bag_filename]["orientation"] && config["initial_pose"][bag_filename]["orientation"]["x"]
        && config["initial_pose"][bag_filename]["orientation"]["y"]
        && config["initial_pose"][bag_filename]["orientation"]["z"]
...
{
  if (!this->CheckParameterExistence_("x0", pose_.x)) pose_.x = 0.0;
  if (!this->CheckParameterExistence_("y0", pose_.y)) pose_.y = 0.0;
  if (!this->CheckParameterExistence_("theta0", pose_.theta)) { pose_.theta = 0.0; }
  ROS_INFO("Initial pose set at (x, y, theta) = (%2f, %2f, %2f).", pose_.x, pose_.y, pose_.omega);
}
...
{
  set_odom_to_zero_srv_ = nh_.advertiseService<std_srvs::Empty::Request, std_srvs::Empty::Response>(
    "set_odometry_to_zero", boost::bind(&OdometryNode::SetOdometryToZero_, this, _1, _2));
  set_odom_to_pose_srv_ = nh_.advertiseService<robotics_hw1::SetOdom::Request, robotics_hw1::SetOdom::Response>(
    "set_odometry_to_pose", boost::bind(&OdometryNode::SetOdometryToPose_, this, _1, _2));
}

bool OdometryNode::SetOdometryToPose_(robotics_hw1::SetOdom::Request &req, robotics_hw1::SetOdom::Response &res)
{
  pose_.x = req.x;
  pose_.y = req.y;
...
bool OdometryNode::SetOdometryToZero_(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
{
  pose_.x = 0.0;
  pose_.y = 0.0;
  pose_.theta = 0.0;
...
void OdometryNode::ChooseIntegrationMethod_(robotics_hw1::IntegrationConfig &config, uint32_t level)
{
...
{
  if (publish_tf_) {
    if (last_tf_time_.has_value() && ros::Time::now() == last_tf_time_) { return; }
...
int main(int argc, char **argv)
{
  ...
  if (FLAGS_bag_filename.empty()) { ROS_FATAL_STREAM("Please provide bag filename"); }
